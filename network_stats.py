...
class NetworkStats:
    ...
    def log_net_device_bytes(self, device, remark=""):
        ...
        sql_command = "insert into network_stats(rx_bytes,tx_bytes,device,remark,created_at)values({0},{1},'{2}','{3}','{4}')".format(
            rx_bytes, tx_bytes, device, remark, time_string)
    ...
    def today_bytes(self):
        ...
        sql_command = "select sum(rx_bytes),sum(tx_bytes) from network_stats where created_at >= '{0}'".format(
            time_string)
    ...
    def this_week_bytes(self):
        ...
        sql_command = "select sum(rx_bytes),sum(tx_bytes) from network_stats where created_at >= '{0}' and created_at <= '{1}'".format(
            start_ts, end_ts)
        rows = get_rows("/etc/net_stats.db", sql_command)
        if len(rows) > 0:
            a = rows[0][0]
